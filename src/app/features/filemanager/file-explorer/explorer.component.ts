...
export class FileExplorerComponent implements OnInit, OnDestroy {
  ...
  this.destroy$.complete();
    ...
    loadFolderContent(folder: any) {
        ...
        if (folder?.key) {
            ...
            isShared = this.keys_to_be_skipped.includes(folder.key) || folder.key == "home"? true : folder.is_shared;
        ...
        this.inner_files = []
        this.fileManagerService.list_files_with_qparameter({'parent': parent, 'isFolder': isFolder, 'isShared': isShared}).pipe(takeUntil(this.destroy$)).subscribe((response)=> {
  ...
  errorToast(msg) {
    ...
    saveRevokePermissions() {
        ...
        if (this.removedFileUsers.length > 0) {
